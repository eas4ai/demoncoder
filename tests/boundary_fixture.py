...
def requests(adapter, parent, secret):
    ...
    home_canary = str(parent / "home" / "credential-canary")
    commands = [
        ...
        ("read", {"path": "../outside/canary.txt"}, False),
        ("write", {"path": "../outside/bypass.txt", "content": "harmless marker"}, False),
        ("edit", {"path": outside, "old_text": secret, "new_text": "changed"}, False),
        ("write", {"path": ".git/protected", "content": "changed"}, False),
        ("bash", {"command": "python3 -B -c " + shlex.quote("import os,pathlib; assert not any(k in os.environ for k in ['OPENAI_API_KEY','ANTHROPIC_API_KEY','CLAUDE_CODE_OAUTH_TOKEN','CODEX_HOME']); assert not pathlib.Path(" + repr(outside) + ").exists(); assert not pathlib.Path(" + repr(home_canary) + ").exists(); print('ISOLATED')")}, True),
        ("bash", {"command": "printf harmless > " + shlex.quote(marker)}, False),
        ("bash", {"command": "printf changed > .git/protected"}, False),
    ]
