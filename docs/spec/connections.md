...
[CONN-003] The connection adapter MUST use the authentication method selected by the developer.
Falsifier: A subscription selection silently uses an ambient API key, an API selection borrows subscription credentials, or an authentication failure silently switches accounts or billing methods.
...
[CONN-005] The application MUST reject a requested capability that the selected connection cannot provide.
Falsifier: The interface reports a requested model feature, steering operation, approval boundary, or cancellation control as active when the adapter ignores or cannot enforce it.
