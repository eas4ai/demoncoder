...
Status: Historical proposal; confirmed 2026-09-07. Contract: docs/spec/evidence-based-improvement.md.
Prefix: LEARN

The developer selected the next roadmap commitment after status and decision
remediation was completed and installed. This document proposes its detailed
behavior and falsifiers for agreement. It does not yet authorize implementation.
...
## Proposed requirements and falsifiers
...
Proposed mechanism: evidence-based-improvement, with per-requirement results from
...
This review attacked citation loss, duplicate authorization, confusing acceptance
with benefit, granting lessons instruction authority, irrelevant retrieval,
cross-workspace leakage, hidden proposal calls, missing storage limits and
unsupported external-backend recovery claims. Each has a corresponding falsifier
above. The integration demonstration must use production-created evidence and
actual subsequent provider requests; preloaded synthetic lesson context alone
cannot establish the commitment.

The proposal deliberately preserves the existing native-parent workflow limit.
Lesson approval is distinct from task acceptance because a successful patch need
not justify a reusable rule. Matching must be explainable because an unrelated
lesson can degrade otherwise correct work. The exact storage layout, identifiers,
commands and finite bounds remain implementation decisions after agreement.
No runtime behavior or new mechanism has been implemented or verified here.
