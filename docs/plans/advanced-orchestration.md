...
- [x] Add optional backward-compatible orchestration state to each AgentRecord:
...
- [x] Add optional orchestration identity to DelegationIdentity containing the
...
- [x] Generalize the existing tool-free review helper with a trusted role enum:
...
- [x] Exercise the role helper with a forbidden tool request, invalid verdict,
...
- [x] Add `--orchestrate --judge CONNECTION`, requiring enabled child connections,
...
- [x] Extend the parent delegate schema with optional `depends_on: [ID, ...]` only
...
- [x] In the shared durable update, select eligible queued IDs and transition
...
- [x] After a worker completes, inspect the current child patch and execute checks
...
- [x] When correction is required, persist the next admitted round before a
...
- [x] Preserve the explicit integration gate, including current passing checks,
...
- [x] Render assignment IDs, waiting causes, supervision roles and correction
...
- [x] First run the actual binary with the new opt-in and retain its missing-option
...
- [x] ORCH-001/002: queue independent and dependent assignments with delayed work,
...
- [x] ORCH-003/004: inspect advisor source/check inputs and original worker/judge
...
- [x] ORCH-005/006: persist two corrective file mutations and deny a third; exhaust
...
- [x] ORCH-007: kill during waiting, advisor, worker response, judge and correction.
...
- [x] The shell mechanism runs prerequisite Rust tests and all seven production
...
- [x] Run the full Rust suite, strict Clippy, formatting and affected existing
  ...
  `DEMONCODER_TEST_BINARY=/home/shawn/.cargo/bin/demoncoder`.

Completion condition: finish only on Cairn Done with completed installed
verification. Cairn receipt collection remains a separate final gate.
